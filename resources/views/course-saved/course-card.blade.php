...
<div class="row no-gutters border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-200 position-relative">
  ...
  <div class="col p-4 d-flex flex-column position-static justify-content-between">
    ...
    <div class="d-flex justify-content-between align-items-center ">
      <h6>Category:<a class="text-decoration-none" href="{{ route('course', ['category' => $course->category]) }}">
          {{ $course->category->name }}</a></h6>
      <small class="mb-1 text-muted">Latest updated on {{ $course->updated_at->format('Y/m/d') }}</small>
    </div>
  </div>
  <div class="btn-group align-items-start py-4 px-3">
    ...
    <a href="{{ route('bookmark.save', ['course' => $course]) }}"
